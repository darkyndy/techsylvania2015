var DyCache = (function () {
  "use strict";
  /*global window: true, document: true, navigator: true, console: true, confirm: true*/
  var obj,
    el = null;
  ...
  function handleCacheEvent(e) {
    ...
    if (eventType === "cached") {
      ...
      newContent = "downloading resource " + e.loaded + " from " + e.total;
  ...
  /**
   * @description Receiving object where events should be attached
   *  Attach all events from array with callback provided
   * @param {Object} obj
   * @param {Array} eventsArr
   * @param {Function} eventCb
   */
  function addMultipleEvents(obj, eventsArr, eventCb) {
    var i,
      eventsArrLength;
    eventsArrLength = eventsArr.length;
    for (i = 0; i < eventsArrLength; i += 1) {
      obj.addEventListener(eventsArr[i], eventCb);
    }
  }
  /**
   * Attach cache events
  ...
  function attachCacheEvents() {
    if (window.applicationCache) {
      ...
      addMultipleEvents(obj, ["cached", "checking", "downloading", "error", "noupdate", "obsolete", "progress", "updateready"], handleCacheEvent);
      /*try {
       obj.update();
       } catch(ex) {
